...
public class FileUtil {
	...
	public static void readLines(String file, ArrayList<String> lines) {
		...
		try {
			...
			reader = new BufferedReader(new FileReader(new File(file)));//File是文件的绝对地址
	...
	/**
	 * Create a directory by calling mkdir();//检测结果存放目标文件夹是否存在，存在继续，不存在创造一个
	 * 
	 * @param dirFile
	 */
	public static void mkdir(File dirFile) {
		try {
			...
			boolean bFile = dirFile.exists();  //测试目标地址所指向的文件夹是否存在
			if (bFile == true) {
