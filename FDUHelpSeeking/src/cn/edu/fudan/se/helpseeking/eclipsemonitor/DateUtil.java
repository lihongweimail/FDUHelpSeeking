...
public class DateUtil {
	...
	/**
	 * @return Time formatted according to:
	 *         http://www.iso.org/iso/date_and_time_format
	 */
	public static String getIsoFormattedDateTime(Calendar calendar) {
		return getIsoFormattedDate(calendar)
				+ "T" + calendar.get(Calendar.HOUR) + "-" + calendar.get(Calendar.MINUTE) //$NON-NLS-1$ //$NON-NLS-2$
				+ "-" + calendar.get(Calendar.SECOND); //$NON-NLS-1$
	}

	/**
	 * Returns the time in the format: HHH:MM. If <code>includeSeconds</code> is
	 * true, the returned format is: HHH:MM:SS.
	 * 
	 * @since 3.5
	 */
	...
	private enum Period {
		...
		MONTH(MILLIS_MONTH, Messages.DateUtil_month_single,
				Messages.DateUtil_month_multi), //
		...
		MINUTE(MILLIS_MINUTE, Messages.DateUtil_minute,
				Messages.DateUtil_minutes), //
		SECOND(MILLIS_SECOND, Messages.DateUtil_second,
				Messages.DateUtil_seconds);
	...
	private static class PeriodString {
		...
		public String toString() {
			return period.toString(duration)
					+ ((next != null) ? " " + next.toString() : ""); //$NON-NLS-1$ //$NON-NLS-2$
		}
	...
	public static String getRelativeDuration(long diff) {
		...
		for (Period period : Period.values()) {
			...
			// only return more than one segment if the second segment follows
