...
public class HelpSeekingBrowserView extends ViewPart {
	...
	public void createPartControl(Composite arg0) {
		...
		Text filetrLabel = new Text(urlComposite, SWT.MULTI | SWT.WRAP
				| SWT.None);
		filetrLabel.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				true, 1, 1));
		...
		Text unselectLabel = new Text(urlunselectComposite, SWT.MULTI
				| SWT.WRAP | SWT.None);
		unselectLabel.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				true, 1, 1));
		unselectLabel.setText("Unselelct Results:");

		urlTreeUnselect = new Tree(urlunselectComposite, SWT.BORDER);
		urlTreeUnselect.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				true, 9, 1));
...
//	IWebBrowser browser;
	...
	final String highlightJScode = FileHelper.getHTMLContent(CommUtil
			.getFDUHelpseekingPluginWorkingPath() + "/javascripthiglight.js");
...
//	  
			...
			newbrowserid
					.setId("OpenWebsite" + String.valueOf(currentBrowserID));

			FDUHelpSeekingPlugin pluginInstance = FDUHelpSeekingPlugin
					.getINSTANCE();
			...
			if ((historyUrlSearch.get(currentHistoryUrlSearchID).getTopicId()) != null)
				tid = historyUrlSearch.get(currentHistoryUrlSearchID)
						.getTopicId();

			nwuiInfo.setTopicId(tid);
			nwuiInfo.setTopicName(historyUrlSearch.get(
					currentHistoryUrlSearchID).getTopicName());
			nwuiInfo.setOpenTime(new Timestamp(System.currentTimeMillis()));
			DatabaseUtil.addNewWebUseInfo(nwuiInfo);

			System.out.println("browser id: " + "OpenWebsite"
					+ String.valueOf(currentBrowserID));

			// browser.openURL(new URL((urls.getUrl())));
			// browser.openURL(new
			// URL("file:///Users/Grand/Downloads/testforhighlightdo.html")); //
			// 2.2测试本地文件
			...
			System.out.println("the file name of URL is:  "
					+ openUrl.getFile().toLowerCase().toString());
			final String openUrlFilepath = CommUtil
					.getFDUHelpseekingPluginWorkingPath()
					+ "/"
					+ String.valueOf(currentBrowserID) + ".html";

			final String openUrlFileWithProtocolString = "file://"
					+ openUrlFilepath;
			final URL openUrlFileWithProtocol = new URL(
					openUrlFileWithProtocolString);
			...
			try {
				...
				browserPart = (BrowserEditor) activePage.openEditor(
						new NullEditorInput(),
						"cn.edu.fudan.se.helpseeking.editors.BrowserEditor");
			...
			Job job = new Job("jobname获取数据WEB页面") {
				protected IStatus run(IProgressMonitor monitor) {
					// 在此添加获取数据的代码
//2015.2.11 
//					考虑后续在这里添加代码，判断预取的RUL对应的时那种网页类型，如果是html目前可以直接使用browserexecute
//					 如果是doc或 pdf文档，需要调用 对应文件类型的处理 jar包  实现定位和高亮这些词汇。
...
//					doc = Jsoup.connect(url).timeout(200000).get();
					...
					/*
					 * String kwcjsString=""; String keywordsandcolorsJScode=
					 * "function onloadinghighlight()\r\n" + "{\r\n"; // +
					 * " searchPrompt(\"unpredictable\", false, \"pink\", \"yellow\");\n"
					 * // +
					 * " searchPrompt(\"highlight\", true,\"red\",\"yellow\");\n"
					 ...
					 * String labels =
					 * foamtreeKeyWordsSet.get(i).getKeywordName(); labels=
					 * CommUtil.getNewSimpleWords(labels); String colors =
					 * foamtreeKeyWordsSet.get(i).getKeywordColorName();
					 * //如何求colors的对比色，反色呢？ if (labels.contains(".")) { labels =
					 * labels.replaceAll("[.]", " "); }
					 * 
					 * if (labels.contains("(")) { labels =
					 * labels.replaceAll("[(]", " "); } if
					 * (labels.contains(")")) { labels =
					 * labels.replaceAll("[)]", " "); }
					 * 
					 * kwcjsString=kwcjsString+" searchPrompt(\""+labels+
					 * "\", false, \""+colors+"\", \"LightYellow\");\r\n"; }
					...
					// try {
					//
					// doGenHighlightHTML(urlString, true, highlightJScode,
					// keywordsandcolorsJScode, openUrlFilepath);
					//
					// } catch (IOException e) {
					// // TODO Auto-generated catch block
					// e.printStackTrace();
									...
									.addProgressListener(
											new ProgressListener() {
												public void completed(
													...
													System.out
															.println("FOR HIGHLIGHT , Page Loading ... ");
													boolean result = browserPart
															.getMybroBrowser()
															.getBrowser()
															.execute(
																	GenerateJS());
													if (!result) {
														System.out
																.println("Browser open webpage failed!");
...
//		buffer.append("i = -1;} } } } \r\n");
//        buffer.append("document.body.innerHTML = newText;  \r\n");		
...
//			String colors = foamtreeKeyWordsSet.get(i)
//					.getKeywordFontColorName();
//			String backgroundColors = foamtreeKeyWordsSet.get(i)
//					.getKeywordColorName();
//
//			// 如何求colors的对比色，反色呢？
//			if (labels.contains(".")) {
...
//			}
//
//			labels = CommUtil.removeDuplicateWordsWithBlankSplit(CommUtil
//					.stringToList(labels.trim(), "[ ]"));
//
//			// buffer.append(" searchPrompt('"+labels.trim()+"',false,'"+colors+"','Yellow');\r\n");
//            System.out.println("highligt string:"+labels);
//			buffer.append("var searchArray = labels.split(\" \"); \r\n");
//			buffer.append("for (var j = 0; j < searchArray.length; j++) { \r\n");
//	        buffer.append("bodyText = doHighlight(bodyText, searchArray[j],'" + colors + "','" + backgroundColors + "');} \r\n");
//	  
...
//		    }
		...
		// 替换方案 宏伟方案
		 ...
		 String
		 backgroundColors=foamtreeKeyWordsSet.get(i).getKeywordColorName();
		 
		 System.out.println("the colors:= "+colors+"\n  the backgoundcolors:= "+ backgroundColors);
		 ...
		 String[] searchArray=labels.split(" ");
...
//	        buffer.append("bodyText = doHighlight(bodyText, searchArray[j],'" + colors + "','" + backgroundColors + "');} \r\n");
		 ...
		 buffer.append("bodyText = document.body.innerHTML;\r\n");
		
		 buffer.append("return bodyText;\r\n");
		 ...
		 buffer.append("  }\r\n");
		 buffer.append("} \r\n");
//		 buffer.append("alert(\"TEst, for some reason\"); \r\n");
	...
	public static void doGenHighlightHTML(String url, Boolean isUrl,
			String codeString, String highlightwordscolorsCode,
		...
		Writer writer = new PrintWriter(newfilename, charset);
		writer.write(html);
		writer.close();
	}
	...
	public void genUrlTree(String currentTopicName, List<WEBPageBean> list,
			List<KeyWord> searchList, String searchId, String topicId,
			List<KeyWord> foamtreeKeyWords) {
	...
	public void doGenUrlTree(Tree myTree, List<WEBPageBean> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			urlTreeItem
					.setText((i + 1) + ":  " + list.get(i).getTitle().trim());

			urlTreeItem.setForeground(Display.getDefault().getSystemColor(
					SWT.COLOR_BLACK));
			...
			TreeItem urlTreeItemofItemtitle = new TreeItem(urlTreeItem,
					SWT.NONE);
			urlTreeItemofItemtitle.setData(urr);
			urlTreeItemofItemtitle.setText(list.get(i).getUrl().trim());

			TreeItem urlTreeItemofItemsummary = new TreeItem(urlTreeItem,
					SWT.NONE);
	...
	public String genHightLightStr(WEBPageBean webPageBean,
		...
		for (int i = 0; i < searchList.size(); i++) {
			...
			String[] tempsStrings = searchList.get(i).getKeywordName()
					.split("[ .()]");
			if (tempsStrings != null)
				if (tempsStrings.length > 0) {
					for (int j = 0; j < tempsStrings.length; j++) {
						...
						if (!temps.equals("")) {
							if (webPageBean.getTitle().toLowerCase().trim()
									.contains(temps)
									|| webPageBean.getSummary().toLowerCase()
											.trim().contains(temps)
									|| webPageBean.getContent().toLowerCase()
											.trim().contains(temps)) {
